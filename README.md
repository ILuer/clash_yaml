# ClashFreeSub

[![License](https://img.shields.io/github/license/ILuer/clash_yaml)](https://github.com/ILuer/clash_yaml/blob/main/LICENSE)
[![TG](https://img.shields.io/badge/Telegram-群组-blue?logo=Telegram)](https://t.me/+vvqaLx7Mcgc2NmJl)

### 🚀 Clash多终端订阅适配（Clash Mi）

> 订阅节点自动更新，支持多种客户端格式

[<img src="https://clashmi.app/img/app_iconx_256.png" height="160">](https://clashmi.app/img/app_iconx_256.png)

---

## ⭐ 欢迎 Star

如果项目有帮到你，还请右上 **Star** 一下，谢谢！

---



## ☕ 打赏支持

| 微信 |
|:---:|
| ![微信](Donation/WeChatReward.png) |
[<img src="/Donation/WeChatReward.png" height="160">](Donation/WeChatReward.png)


---

## 📥 订阅地址

> 💡 推荐使用加速链接，国内直连更稳定

### Clash / Mihomo

```
https://gh-proxy.com/raw.githubusercontent.com/ILuer/clash_yaml/main/clash_fallback.yaml
```

<details>
<summary>备用地址（原始链接）</summary>

```
https://raw.githubusercontent.com/ILuer/clash_yaml/main/clash_fallback.yaml
```

</details>

---

## 📖 使用说明

| 步骤 | 操作 |
|:---:|------|
| 1️⃣ | 复制上方对应客户端的订阅地址 |
| 2️⃣ | 打开代理客户端，导入订阅 |
| 3️⃣ | 选择节点，开启代理 |

---

## ⚠️ 免责声明

- 本项目仅供学习交流，用于查找资料、学习知识
- 请勿用于任何违法行为，否则后果自负
- 所有资源均来自互联网，本仓库仅做配置示例
- 如有侵权，请提 [Issue](https://github.com/ILuer/clash_yaml/issues)，将及时删除

---

## 📈 Star History

[![Star History Chart](https://api.star-history.com/svg?repos=Ruk1ng001/freeSub&type=Date)](https://star-history.com/#Ruk1ng001/freeSub&Date)
